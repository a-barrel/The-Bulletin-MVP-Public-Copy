 ...
 *
 * This “ProfilePage_debug” component is the legacy profile implementation that still
 * contains all of the existing business logic: moderation flows, DM creation, stats,
 * raw data views, etc. The “pretty” profile UI (ProfilePage.jsx) is intentionally slimmer,
 * so any new behaviour that gets added to the main profile view should also be kept in sync
 * here until we finish consolidating the two experiences.
 */
...
import './ProfilePage_debug.css';

export const pageConfig = {
  id: 'profile-debug',
  label: 'Profile (Debug)',
  icon: AccountCircleIcon,
  path: '/profile-debug/:userId',
  order: 191,
  showInNav: true,
  protected: true,
  resolveNavTarget: ({ currentPath } = {}) => {
    if (!runtimeConfig.isOffline) {
      return routes.profileDebug.me;
    }
    ...
    if (/^profile-debug\/.+/i.test(sanitized)) {
      return `/${sanitized}`;
    }
    if (/^\/profile-debug\/.+/i.test(trimmed)) {
      return trimmed;
    }
    return routes.profileDebug.byId(sanitized);
  }
};
