...
export default function App() {
  ...
  const currentNavPath = useMemo(() => {
    ...
    const matched = navPages.find((page) => {
      ...
      return aliases.some(
        (alias) =>
          alias && matchPath({ path: alias, end: alias === '/' }, location.pathname)
      );
    });
  ...
  useEffect(() => {
    ...
    setNavHistory((prev) => {
      ...
      if (pendingBackTarget) {
        ...
        const baseHistory =
          targetIndex !== -1 ? prev.slice(0, targetIndex + 1) : prev;
      ...
      navPages.find(
        (page) =>
          ...
          (Array.isArray(page.aliases) &&
            page.aliases.includes(previousNavPath))
      ) ?? null
    );
    ...
    <NavOverlayProvider value={navOverlayContextValue}>
      <ThemeProvider theme={theme}>
              ...
              >
                <Stack spacing={1.5}>
                  ...
                  {navPages.length > 0 ? (
                    ...
                    <Typography variant="body2" color="text.secondary">
                      Add a new page under `src/pages` with `showInNav: true` to
        ...
        <Routes>
          ...
          {/* Routes from `main` for other dev pages, filtering out duplicates */}
          {pages
            .filter((page) => !explicitPaths.has(page.path))
            .map((page) => (
              <Route
                key={page.id}
                path={page.path}
          ...
          {pages.map((page) =>
            page.aliases
              .filter((alias) => !explicitPaths.has(alias))
              .map((alias) => (
                <Route
